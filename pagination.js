  ...
  function processPage() {
    if (window.location.pathname.includes('/search')) {
      ...
      const allPosts = Array.from(document.querySelectorAll('.post-outer, .post.hentry')).map(post => {
        const dateElement = post.querySelector('.date-header span, .post-timestamp, .published, time');
        let date = null;
        if (dateElement) {
  ...
  function attachPagerListener() {
    ...
    if (pagerLink && !pagerLink.dataset.listenerAttached) {
      ...
      if (posts.length > 0) {
        ...
        const dateElement = lastPost.querySelector('.date-header span, .post-timestamp, .published, time');
